// LocalPortNodeV2.tsx - 基于节点管理器的本地端口节点组件
...
// 扩展基础节点数据，添加本地端口特定数据
export interface LocalPortNodeV2Data extends BaseNodeData {
  port: Port;
  projectId: number;
  groupId: number;
  isExpanded?: boolean;
 ...
 * 本地端口节点V2组件
 */
const LocalPortNodeV2Component = memo(function LocalPortNodeV2({ id, data, selected }: NodeProps) {
  const {
    port,
    projectId,
    groupId,
    ...
    onStop,
    onRestart,
  } = data as LocalPortNodeV2Data;

  // 使用节点生命周期钩子
  useNodeLifecycle(id, data, {
    onMount: (nodeId, data) => {
      console.log(`本地端口节点 ${nodeId} 已挂载`, data);
    },
    onUpdate: (nodeId, oldData, newData) => {
      console.log(`本地端口节点 ${nodeId} 数据已更新`, { oldData, newData });
    },
    onUnmount: (nodeId) => {
      console.log(`本地端口节点 ${nodeId} 已卸载`);
    },
  });
  ...
  return (
    <div className="local-port-node-v2">
      {/* 输入连接点 */}
      <Handle
        type="target"
        position={Position.Left}
        style={{ 
          background: port.color || '#3b82f6',
      ...
      <Card 
        ...
        style={{ 
          borderTopColor: port.color || '#3b82f6',
          borderTopWidth: '3px',
        }}
      >
...
// 创建节点配置
const localPortNodeV2Config = createNodeConfig({
  type: 'localPortV2',
  displayName: '本地端口 V2',
  description: '本地端口转发节点，支持本地端口映射和管理',
  icon: React.createElement(Monitor, { className: "w-4 h-4" }),
  categoryId: 'network',
  version: '2.0.0',
  tags: ['port', 'local', 'tunnel', 'forward'],
  resizable: true,
  connectable: true,
  deletable: true,
  draggable: true,
  ...
  createNodeData: (params: {
    ...
    description: params.description || params.port.description || '本地端口节点',
  ...
  validateNodeData: (data: any) => {
    return !!(
      data &&
      data.id &&
      data.type === 'localPortV2' &&
      data.port &&
      data.port.id &&
      data.port.name &&
      data.port.type === 'local' &&
      data.projectId &&
      data.groupId
    );
  }
...
// 导出带包装器的组件，并附加配置元数据
export const LocalPortNodeV2 = attachNodeConfig(
  withNodeBase(LocalPortNodeV2Component),
  localPortNodeV2Config
);

// 使用装饰器注册节点（用于主动注册）
registerNode({
  ...localPortNodeV2Config,
  component: LocalPortNodeV2
});

export default LocalPortNodeV2;
